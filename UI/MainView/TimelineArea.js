 var Component=fabric.util.createClass({
            ...
            if(this.localState.coords.left<this.globalState.padding.width-(this.localState.coords.width/2)){this.localState.coords.left=this.globalState.padding.width-(this.localState.coords.width/2)}
            if(this.localState.coords.left>this.globalState.timingDistances.scrollWidth-this.globalState.padding.width-this.localState.coords.width/2){this.localState.coords.left=this.globalState.timingDistances.scrollWidth-this.globalState.padding.width-this.localState.coords.width/2}
        },
            render:function(ctx){
            ...
            let mouseXInWorldCoords=e.clientX+this.globalState.timingDistances.scrollLeft;
            ...
            if(this.localState.coords.left<this.globalState.coords.left){this.localState.coords.left=this.globalState.coords.left}
            if(this.localState.coords.left>this.globalState.coords.width-this.localState.coords.width){this.localState.coords.left=this.globalState.coords.width-this.localState.coords.width}
            ...
            if(buttonACoords.left<this.globalState.coords.left){buttonACoords.left=this.globalState.coords.left};
            if(buttonACoords.left>buttonBCoords.left+buttonBCoords.width-this.minWidth){buttonACoords.left=buttonBCoords.left+buttonBCoords.width-this.minWidth}
            ...
            let buttonACoords=this.buttonA.localState.coords;
            let buttonBCoords=this.buttonB.localState.coords;
            if(buttonBCoords.left+buttonBCoords.width>this.globalState.coords.width){buttonBCoords.left=this.globalState.coords.width-buttonBCoords.width}
            if(buttonBCoords.left<buttonACoords.left+this.minWidth-buttonBCoords.width){buttonBCoords.left=buttonACoords.left+this.minWidth-buttonBCoords.width;}
            ...
            components:[],
            globalState:{
            coords:{left:null,top:null,width:null,height:null,}, // width: ancho viewport , totalWidth:ancho total (partes ocualtas incluidas)
            time:{segmentTimeValue:100,minSegmentTimeValue:100,duration:60000},
            timingDistances:{minSegmentDistance:100,maxSegmentDistance:200,segmentDistance:0,scrollWidth:0,scrollLeft:0},
            padding:{width:50}
        },
            ...
            this.HTMLElement=document.querySelector("#c");
            ...
            this.setupGlobalStateTiming();
            this.initComponents();
            this.setupComponentsCoords();

            EventManager.registerOnMouseDown(this);
            EventManager.registerOnMouseUp(this);
            EventManager.registerOnMouseMove(this);
            ...
            this.globalState.coords.left=boundingRect.left;
            this.globalState.coords.top=boundingRect.top;
            this.globalState.coords.width=this.HTMLElement.clientWidth;
            this.globalState.coords.height=this.HTMLElement.clientHeight;

        },
            setupGlobalStateTiming:function(){
            this.calcGlobalStateTiming((this.globalState.coords.width-(this.globalState.padding.width*2)));
        },
            calcGlobalStateTiming:function(widthArea){//debe ser el ancho del area USABLE (sin contar los paddings)
            let numSegmentsWithBaseSegmentTimeValue=this.globalState.time.duration/this.globalState.time.minSegmentTimeValue;
            ...
            newSegmentTimeValue=this.globalState.time.minSegmentTimeValue       *   Math.pow(2,auxExponent);
            ...
            newSegmentTimeValue=this.globalState.time.minSegmentTimeValue;
        }
            /**/
            ...
            this.keysBarComponent=new TimeLineKeysBar(this,this.globalState,this.mouseState,{"position":["left","top"],"scale":["scaleX","scaleY"],"rotation":["angle"],"opacity":["opacity"]});
            ...
            /*references*/
            this.keysBarComponent.setMarkerComponent(this.markerComponent);
            /*eventos*/
        ...
        }else{alert("erorrr cant argumetnso setGlobalStateValue");}
            ...
            notificationOnMouseMove:function(e){
            this.mouseState.x=e.clientX-this.globalState.coords.left;
            this.mouseState.y=e.clientY-this.globalState.coords.top;
            this.mouseState.inWorldX=e.clientX-this.globalState.coords.left+this.globalState.timingDistances.scrollLeft;
            this.mouseState.inWorldY=e.clientY-this.globalState.coords.top;
            for (let i = 0; i < this.components.length; i++) {
            ...
            notificationOnScrollBarResize:function(scrollWidth){
            let percentViewport=scrollWidth/this.globalState.coords.width;
            let usableScrollWidth=(this.globalState.coords.width-this.globalState.padding.width*2)/percentViewport;

            this.calcGlobalStateTiming(usableScrollWidth);
        },
            ...
            initialize:function(canvas,globalState,mouseState,properties){
            this.callSuper('initialize',globalState,mouseState);
            this.canvas=canvas;
            ...
            setMarkerComponent:function(markerComponent){
            this.markerComponent=markerComponent;
            for(let key in this.dictPropertiesLanes){
            this.dictPropertiesLanes[key].setMarkerComponent(this.markerComponent);
        }
        },
            initComponents:function(){/*{"position":[],"scale":[],"rotation":[]}*/
            for(let key in this.MODELproperties){
            let lane=new KeyBarPropertyLane(this.canvas,this.globalState,this.mouseState,this.MODELproperties[key],key);
            ...
            this.dictPropertiesLanes[key]=lane;
            ...
            findKeyframesInsideSelection:function(){
            ...
            if(this.selectedKeyframes.length>0){
            for(let i=0;i<this.selectedKeyframes.length;i++){
            this.dictPropertiesLanes[this.selectedKeyframes[i].propertyLaneName].discartKeyFrame(this.selectedKeyframes[i]);
        }
            this.selectedKeyframes=[];
        }
            if(this.keyframeSelected){
            this.dictPropertiesLanes[this.keyframeSelected.propertyLaneName].discartKeyFrame(this.keyframeSelected);
            this.keyframeSelected=null;
        }
            this.canvas.requestRenderAll();
        },
            addKeyFrame:function(property){
            this.dictPropertiesLanes[property].retrieveKeyFrame();
        },
            onLocalCoordsSettled:function(){
            this.setupComponentsCoords();
        },
            ...
            onMouseDragEnded:function(){
            this.deselectCurrentSelection();
            this.findKeyframesInsideSelection();
            this.selecting=false;
            this.notifyOnKeyframeSelectionUpdated();
            this.canvas.requestRenderAll();
        },
            ...
            notificationOnKeyframeFixedClick:function(keyframe){//click sin dragging, si se hizo dragging no se notifica
            if(this.selectedKeyframes.length!==0){
            this.deselectCurrentSelection();
            keyframe.select();
            this.keyframeSelected=keyframe;
            ...
            initialize:function(canvas,globalState,mouseState,properties,name){
            this.callSuper("initialize",globalState,mouseState);
            ...
            retrieveKeyFrame:function (){
            if(this.counterActiveKeyFrames>=this.keyFrames.length){
            let newKey=new KeyFrame(this.canvas,this.globalState,this.mouseState,this.markerComponent.timelineTime,this.name)
            ...
            this.keyFrames[this.counterActiveKeyFrames].activate(this.markerComponent.timelineTime);
        }
            ...
            notificationOnKeyframeDragging:function(keyframe,offsetX){
            this.observerOnKeyframeDragging.notificationOnKeyframeDragging(keyframe,offsetX);
        },
            notificationOnKeyframeDragEnded:function(keyframe){
            ...
            initialize:function (canvas,globalState,mouseState,time,propertyLaneName){
            this.callSuper("initialize",globalState,mouseState);
            this.localState={colors:{idle:"red",selected:"pink"}}
            this.propertyLaneName=propertyLaneName;
            this.canvas=canvas;
            this.isActive=true;
            ...
            activate:function(timelineTime){
            this.isActive=true;
            ...
            let xInside=false;
            let yInside=false;
            if(width>0){if(localCoordsLeft>startX && localCoordsLeft<endX){xInside=true}}
            else{if(localCoordsLeft>endX && localCoordsLeft<startX){xInside=true}}

            if(height>0){if(localCoordsTop>startY && localCoordsTop<endY){yInside=true}}
            else{if(localCoordsTop>endY&& localCoordsTop<startY){yInside=true}};
